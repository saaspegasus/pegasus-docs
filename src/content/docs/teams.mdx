...
For Celery tasks that operate on team-scoped data, use the `@team_task` decorator from `apps.teams.celery`.
...
The convention is that the first positional argument to the task (after `self` when `bind=True`) is a `team_id`:
...
Enqueue it like any other Celery task, with `team_id` as the first positional argument:

```python
process_team_data.delay(team.id, payload.id)
```
