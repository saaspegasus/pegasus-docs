...
From version 2026.6, Pegasus includes a `@team_task` decorator (in `apps.teams.celery`)
for team-scoped Celery tasks. It runs the task body inside a team context — so anything inside the task
that uses the [team context variable](#team-context-variable) or `MyTeamModel.for_team` just works.
...
def my_task(self, team_id, other_id):
    # team is automatically added to the query here, similar to views
    objs = MyTeamModel.for_team.filter(...)
    ...

# can be called either of these ways
my_task.delay(team.id, payload.id)            # positional
my_task.delay(team_id=team.id, other_id=...)  # kwarg
```

If the team can't be loaded, the task raises `Team.DoesNotExist`, since it is explicitly expected
to be run in the context of a team.
