...
By default, it is included in your `app_base.html` file and will be included in any page that extends it
(most of the logged-in pages in the app).

#### Adding the chat widget to a page

To add the chat widget to a page, you can follow the example in `app_base.html`.
There are two steps:

First, include the component at the end of the `<body>` tag.
If you're extending the `base.html` file this will be at the end of the `{% block body %}` block.
...
#### Removing the chat widget

To remove the chat widget from your app, delete the lines mentioned above from your `app_base.html` file.
