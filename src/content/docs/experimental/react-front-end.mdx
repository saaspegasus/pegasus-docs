...
and uses React with TailwindCSS and [shadcn/ui](https://ui.shadcn.com/) for components.
...
This frontend uses [shadcn/ui](https://ui.shadcn.com/) for UI components.
shadcn components are copied into the codebase (not installed as a package) and can be customized directly.
...
shadcn colors are mapped to DaisyUI theme variables in `src/index.css` under `@theme inline`.
This ensures shadcn components use the same colors as the rest of the app.
