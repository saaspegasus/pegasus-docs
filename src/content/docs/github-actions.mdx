...
description: Automate Django testing and front-end builds with GitHub Actions CI/CD workflows for continuous integration.
---
...
By default, it will:
 ...
 - Run on Python version <PythonVersion/> (other Python versions can be added by modifying the `python-version` list)
...
By default, it will:
 - Run on every push to the `main` branch and every pull request.
 - Run on Node version <NodeVersion/> (other Node versions can be added by modifying the `node-version` list).
