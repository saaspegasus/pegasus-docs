...
This release adds an expiration window to team invitations, significantly expands the subscription
test suite, and includes a handful of other bug fixes and improvements.
...
- **Team invitations now expire.** Invitations track when they were last sent and stop working after
  ...
  Resending an invitation renews it. Related changes:
  - Added a `sent_at` field to the `Invitation` model, which requires a migration.
  ...
  - The team management page now shows expiry information.
  - The invitation `resend` API endpoint now returns the updated invitation, and the React UIs use it directly.
- **Expanded the subscription test suite**, including coverage of product metadata, models, feature gating,
  webhooks, wrappers, sync helpers, and background tasks. Thanks Justin for the suggestion!
- Added a "Starting the app" section to the project README covering how to start and stop the app
  after initial setup. Thanks Telmo for suggesting!
- Added a daily Celery task to clear expired sessions from the database (projects that use Celery only).
  This optional cleanup prevents the `Session` table from growing unbounded. Thanks Peter for the suggestion!

### Fixed

- Invited users no longer receive a redundant email confirmation on sign up. 
  The invited email was already being marked as verified (the invitation link itself proves ownership of the email),
  but this happened after allauth had already sent its confirmation email.
  Thanks Peter for reporting!
- The teams middleware now only writes the team to the session when it changes,
  avoiding an unnecessary session save on every request. Thanks Peter for reporting!
- Removed an unused code path from the price lookup helper in `apps/subscriptions/metadata.py`.
- Fixed a typo in a variable name in the API schema filter. Thanks Brennon for reporting!
- Removed the `single_team:resend_invitation` url and view from React builds (which never used it).

### Upgrading

The new `sent_at` field on the `Invitation` model defaults to the current time,
so a migration generated with `makemigrations` will set it on every existing invitation
with the time the migration was *run* rather than when the invitation was actually created.
This means that by default all existing invitations will expire 14 days from whenever the upgrade's
migrations run.

If you'd rather pin the invitation expiry to `created_at` for existing invitations, you can backfill
`sent_at` from `created_at` by modifying the generated migration to look something like this:
...
class Migration(migrations.Migration):
    dependencies = [
        # this line will be different depending on your project's migration history
