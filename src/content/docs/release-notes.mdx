...
This release updates the `pydandatic-ai` dependency to `pydantic-ai-slim[openai,anthropic,google,mcp]>=1.0`
...
The affected `mistral` package was downstream of `pydantic-ai` so theoretically could effect Pegasus projects.
However, in practice it is very unlikely because:

1. Pegasus's lockfile never contained the affected version (released May 7).
   So you'd only be vulnerable if you intentionally upgraded your packages outside of a Pegasus update between May 7 and May 12, 2026 (the last 5 days as of this release).
2. You wouldn't have triggered the vulnerability unless you were intentionally using mistral or set something like `DEFAULT_AI_MODEL=mistral` in your settings/env.
