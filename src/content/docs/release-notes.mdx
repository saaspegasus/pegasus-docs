...
- **Temporarily added a workaround to `pyproject.toml` to allow installing `django-celery-beat` alongside Django 6.0.**
...
All Pegasus dependencies *except* `django-celery-beat` can be installed automatically against Django 6.0.
`django-celery-beat` appears compatible, but hasn't updated the package metadata to allow installing alongside Django 6.0.
Because of this, this version of Pegasus adds the following section to `pyproject.toml` to allow installing `django-celery-beat`:
...
**If you are upgrading an existing project, this line may allow other incompatible dependencies to be installed.**
So you may want to manually check the versions of any non-Pegasus dependencies that get added to ensure they
work correctly.

This configuration section will be removed [as soon as django-celery-beat supports Django 6](https://github.com/celery/django-celery-beat/issues/977).
...
For release notes for earlier releases, see the [Release Notes History](/release-notes-history/) page.
