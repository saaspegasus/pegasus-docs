...
Notes:
- Existing projects will stay on `pg-` classes by default.
  See the migration guide for details on how to migrate your project to native classes.
...
- **Fixed a bug in fixed-amount discount coupons** where the discount was being applied
  ...
  If you use fixed-amount coupons, review any past transactions and update accordingly.
...
- Ran an AI-assisted security and bug audit across the codebase, which surfaced only
  ...
  - Replacing `assert` statements in the ecommerce and subscription checkout flows with
    proper `PermissionDenied` raises — asserts are stripped when Python runs with `-O`,
    so they shouldn't be used for security checks. Also added structured logging around
    these error paths.
