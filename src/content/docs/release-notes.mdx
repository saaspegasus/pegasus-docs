...
- Improved the built-in `resolve-pegasus-conflicts` Claude Code skill with much better guidance on
  resolving migration conflicts during upgrades—including catching silently-renamed migrations that
  git merges without conflict markers—and a new section on recovering from a partially-applied migration.
